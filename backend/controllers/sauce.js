...
exports.likeSauce = (req, res, next) => {
    ...
    Sauce.findOne({ _id: req.params.id })
        .then(sauce => {
            ...
            // vérifications pour contrôler si l'utilisateur est déjà présent dans les tableaux like/dislike
            if (like === 1 && sauce.usersLiked.includes(req.body.userId)) {
                return res.status(500).json({ message: 'Sauce déjà likée' });
            }
            if (like === -1 && sauce.usersDisliked.includes(req.body.userId)) {
                return res.status(500).json({ message: 'Sauce déjà dislikée' });
            }
